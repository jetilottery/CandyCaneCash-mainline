define(require => {
    ...
    function init() {
        ...
        var gameContainer = app.stage.children[0]; // Add the container at the point directly below the result plaque

        for (var i = 0; i < gameContainer.children.length; i++) {
            if (gameContainer.children[i].name === 'endGameNoPlaqueContainer') {
                gameContainer.addChildAt(_container, i - 1);
                break;
            }
        } // Position it
    ...
    function initEmitters(inOrientation, inContainer, inConfig) {
        ...
        inContainer.emitter = utils.createEmitter(
            ...
            inConfig.bigWinSettings.isAnimated, { startFrame: inConfig.bigWinSettings.startFrame, endFrame: inConfig.bigWinSettings.endFrame, isSingleNumFormat: true, frameRate: inConfig.bigWinSettings.frameRate }
        );
